...
def submit_therapeutic_query(request: QueryRequest, background_tasks: BackgroundTasks):
    job_id = str(uuid.uuid4())
    txagent_jobs[job_id] = {
        "id": job_id,
        "status": "pending"
    }
    background_tasks.add_task(run_therapeutic_query, job_id, request)
    return JobResponse(job_id=job_id, status="pending")

@router.get("/status/{job_id}")
def get_query_status(job_id: str):
    if job_id not in txagent_jobs:
        raise HTTPException(status_code=404, detail="Job not found")
    return txagent_jobs[job_id]

def run_therapeutic_query(job_id: str, request: QueryRequest):
    """
    Runs the AI Copilot query in the background.
    """
    txagent_jobs[job_id]["status"] = "running"
    try:
        ...
        traceback.print_exc()
        txagent_jobs[job_id]["status"] = "failed"
        txagent_jobs[job_id]["error"] = str(outer_e)
